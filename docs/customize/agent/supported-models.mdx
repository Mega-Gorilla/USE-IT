---
title: "Supported Models"
description: "Choose your favorite LLM"
icon: "robot"

---

### Recommendations

- Best accuracy: `O3`
- Fastest: `llama4` on groq
- Balanced: fast + cheap + clever: `gemini-flash-latest` or `gpt-4.1-mini`


### OpenAI [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/gpt-4.1.py)

`O3` model is recommended for best performance.

```python
from browser_use import Agent, ChatOpenAI
from browser_use.agent.config import AgentConfig

# Initialize the model
llm = ChatOpenAI(
    model="o3",
)

# Create agent with the model
agent = Agent(config=AgentConfig(task="...", llm=llm))
```

Add to `config.yaml`:

```yaml config.yaml
llm:
  api_keys:
    openai: your-openai-key
```

Or export `OPENAI_API_KEY`.

<Info>
  You can use any OpenAI compatible model by passing the model name to the
  `ChatOpenAI` class using a custom URL (or any other parameter that would go
  into the normal OpenAI API call).
</Info>

### Anthropic [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/claude-4-sonnet.py)

```python
from browser_use import Agent, ChatAnthropic

# Initialize the model
llm = ChatAnthropic(
    model="claude-sonnet-4-0",
)

# Create agent with the model
agent = Agent(
    task="...", # Your task here
    llm=llm
)
```

Add to `config.yaml`:

```yaml config.yaml
llm:
  api_keys:
    anthropic: your-anthropic-key
```

Or export `ANTHROPIC_API_KEY`.

### Azure OpenAI [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/azure_openai.py)

```python
from browser_use import Agent, ChatAzureOpenAI
from pydantic import SecretStr
import os

# Initialize the model
llm = ChatAzureOpenAI(
    model="o4-mini",
)

# Create agent with the model
agent = Agent(
    task="...", # Your task here
    llm=llm
)
```

Add to `config.yaml`:

```yaml config.yaml
llm:
  api_keys:
    azure: your-azure-api-key
  azure_endpoint: https://your-endpoint.openai.azure.com/
```

Environment override: `AZURE_OPENAI_KEY` and `AZURE_OPENAI_ENDPOINT`.

### Gemini [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/gemini.py)

> [!IMPORTANT] `GEMINI_API_KEY` was the old environment var name, it should be called `GOOGLE_API_KEY` as of 2025-05.

```python
from browser_use import Agent, ChatGoogle

# Initialize the model
llm = ChatGoogle(model='gemini-flash-latest')

# Create agent with the model
agent = Agent(
    task="Your task here",
    llm=llm
)
```

Add to `config.yaml`:

```yaml config.yaml
llm:
  api_keys:
    google: your-gemini-key
```

Or export `GOOGLE_API_KEY`.

### AWS Bedrock [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/aws.py)

AWS Bedrock provides access to multiple model providers through a single API. We support both a general AWS Bedrock client and provider-specific convenience classes.

#### General AWS Bedrock (supports all providers)

```python
from browser_use import Agent, ChatAWSBedrock

# Works with any Bedrock model (Anthropic, Meta, AI21, etc.)
llm = ChatAWSBedrock(
    model="anthropic.claude-3-5-sonnet-20240620-v1:0",  # or any Bedrock model
    aws_region="us-east-1",
)

# Create agent with the model
agent = Agent(
    task="Your task here",
    llm=llm
)
```

#### Anthropic Claude via AWS Bedrock (convenience class)

```python
from browser_use import Agent, ChatAnthropicBedrock

# Anthropic-specific class with Claude defaults
llm = ChatAnthropicBedrock(
    model="anthropic.claude-3-5-sonnet-20240620-v1:0",
    aws_region="us-east-1",
)

# Create agent with the model
agent = Agent(
    task="Your task here",
    llm=llm
)
```

#### AWS Authentication

Add to `config.yaml`:

```yaml config.yaml
aws:
  access_key_id: your-access-key
  secret_access_key: your-secret
  region: us-east-1
```

Or rely on the standard AWS credential chain (environment variables, profiles, IAM roles).

You can also use AWS profiles or IAM roles instead of environment variables. The implementation supports:

- Environment variables (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION`)
- AWS profiles and credential files
- IAM roles (when running on EC2)
- Session tokens for temporary credentials
- AWS SSO authentication (`aws_sso_auth=True`)

## Groq [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/llama4-groq.py)

```python
from browser_use import Agent, ChatGroq

llm = ChatGroq(model="meta-llama/llama-4-maverick-17b-128e-instruct")

agent = Agent(
    task="Your task here",
    llm=llm
)
```

Add to `config.yaml`:

```yaml config.yaml
llm:
  api_keys:
    groq: your-groq-key
```

Or export `GROQ_API_KEY`.

## Ollama

1. Install Ollama: https://github.com/ollama/ollama
2. Run `ollama serve` to start the server 
3. In a new terminal, install the model you want to use: `ollama pull llama3.1:8b` (this has 4.9GB)

```python
from browser_use import Agent, ChatOllama

llm = ChatOllama(model="llama3.1:8b")
```

## Langchain

[Example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/langchain) on how to use Langchain with Browser Use.

## Qwen [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/qwen.py)

Currently, only `qwen-vl-max` is recommended for Browser Use. Other Qwen models, including `qwen-max`, have issues with the action schema format.
Smaller Qwen models may return incorrect action schema formats (e.g., `actions: [{"navigate": "google.com"}]` instead of `[{"navigate": {"url": "google.com"}}]`). If you want to use other models, add concrete examples of the correct action format to your prompt.

```python
from browser_use import Agent, ChatOpenAI

base_url = 'https://dashscope-intl.aliyuncs.com/compatible-mode/v1'
llm = ChatOpenAI(model='qwen-vl-max', base_url=base_url)

agent = Agent(
    task="Your task here",
    llm=llm,
    use_vision=True
)
```

Add to `config.yaml`:

```yaml config.yaml
llm:
  api_keys:
    openai: your-alibaba-key
  base_url: https://dashscope-intl.aliyuncs.com/compatible-mode/v1
```

Or export `OPENAI_API_KEY` and set `BASE_URL` when instantiating `ChatOpenAI`.

## ModelScope [example](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/modelscope_example.py)

```python
from browser_use import Agent, ChatOpenAI

base_url = 'https://api-inference.modelscope.cn/v1/'
llm = ChatOpenAI(model='Qwen/Qwen2.5-VL-72B-Instruct', base_url=base_url)

agent = Agent(
    task="Your task here",
    llm=llm,
    use_vision=True
)
```

Add to `config.yaml`:

```yaml config.yaml
llm:
  api_keys:
    openai: your-modelscope-key
  base_url: https://api-inference.modelscope.cn/v1/
```

Or export `OPENAI_API_KEY` and set `BASE_URL` when instantiating `ChatOpenAI`.

## Other models (DeepSeek, Novita, X...)

We support all other models that can be called via OpenAI compatible API. We are open to PRs for more providers.

**Examples available:**
- [DeepSeek](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/deepseek-chat.py)
- [Novita](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/novita.py)
- [OpenRouter](https://github.com/Mega-Gorilla/USE-IT/blob/main/examples/models/openrouter.py)
